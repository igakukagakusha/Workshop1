...
data_location <- paste0(getwd(),"/Data/NMR_data.xlsx")
#Read the first tab of the data file
metabolomicsData <- read_excel(data_location, 1)
...
data <- cor(metabolomicsData[sapply(metabolomicsData,is.numeric)])
...
# Melt the data frame to convert it into a format suitable for heatmap
df = select_if(metabolomicsData, is.numeric)
df_melted <- melt(df)

# Create the heatmap
ggplot(df_melted, aes(x=variable, y=Var2, fill=value)) + 
  geom_tile(color = FALSE) +
  scale_fill_gradient2(low = "blue", high = "red", mid = "white", 
                       midpoint = (max(df) + min(df)) / 2, 
                       space = "Lab", 
                       name="Value") +
  theme_minimal() + 
...
df_melt <- reshape2::melt(metabolomicsData, id.vars = c("id", "group"), 
                           variable.name = "variable", 
                           value.name = "value")

# Create the heatmap
ggplot(df_melt, aes(x = variable, y = group, fill = value)) + 
  geom_tile() + 
  scale_fill_gradient2(low = "blue", high = "red", mid = "white", 
